...
def extract_text_from_pdf(pdf_path):
    """將 PDF 轉為圖片，並使用 Tesseract OCR 進行文字識別"""
    images = convert_from_path(pdf_path, poppler_path=POPPLER_PATH)
    text = ""
    for img in images:
        text += pytesseract.image_to_string(img, lang="chi_tra")  # 繁體中文 OCR
    return text

def handle_pdf_upload(uploaded_file):
    """處理 PDF 上傳，提取內容並存入 session"""
    file_path = save_uploaded_file(uploaded_file)
    extracted_text = extract_text_from_pdf(file_path)

    st.session_state.extracted_pdf_text = extracted_text  # 存入 session_state
    st.success("📄 PDF 內容提取完成！")

    append_message("system", "PDF 內容已提取，使用者可以直接詢問 PDF 相關問題。")
    debug_log("PDF 內容提取完成，已存入 session_state。")
...
def main():
    ...
    with st.sidebar:
        ...
        # --- PDF Upload ---
        st.sidebar.subheader("📄 上傳 PDF 檔案")
        uploaded_pdf = st.sidebar.file_uploader("請選擇 PDF 檔案:", type=["pdf"])
        if uploaded_pdf:
    ...
    if user_input:
        ...
        with st.spinner("Thinking..."):
            try:
                ...
                # 若 PDF 內容已提取，則將問題與 PDF 內容結合
                if "extracted_pdf_text" in st.session_state:
                    pdf_context = f"\n\n這是從 PDF 提取的內容:\n{st.session_state.extracted_pdf_text}"
                    full_query = user_input + pdf_context
                else:
                    full_query = user_input

                append_message("user", full_query)
