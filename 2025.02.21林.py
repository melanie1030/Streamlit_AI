...
st.markdown("""
    ...
    .stApp {
        ...
        color: #fff; /* 將文字顏色改為白色，以在黑色背景上更易閱讀 */
    }
...
def chat_interface(df):
    ...
    st.markdown("""
        <div style='text-align: center; padding: 2rem 0;'>
            <h1 style='color: #333; font-size: 2.5rem; margin-bottom: 1rem;'>💬 AI 數據分析助手</h1>
            <p style='color: #666; font-size: 1.1rem;'>您的智能數據分析夥伴，隨時為您提供專業的數據洞察</p>
        </div>
    """, unsafe_allow_html=True)
    ...
    with input_container:
        ...
        with col1:
            if st.button("🗑️ 清除", help="清除所有聊天記錄"):
                st.session_state.chat_history = []
                st.rerun()
    
    # 用戶輸入
    with input_container:
        if prompt := st.chat_input("💭 有什麼想問的嗎？我來幫您分析..."):
            # 添加用戶消息到歷史
            ...
            with st.chat_message("assistant", avatar="🤖"):
                try:
                    with st.spinner("🤔 思考中..."):
                        # 準備系統消息
                        system_message = """你是一個專業的數據分析助手，專門協助分析零售數據。
                        請根據提供的數據回答問題，並給出具體的數據支持。
                        回答要簡潔明瞭，重點突出。"""
                        ...
                        數據概況：
                        - 總記錄數：{len(df)} 條
                        - 總銷售額：¥{df['Item_MRP'].sum():,.2f}
                        - 平均銷售額：¥{df['Item_MRP'].mean():,.2f}
                        - 商品種類數：{df['Item_Type'].nunique()} 種
                        - 門店數量：{df['Outlet_Identifier'].nunique()} 家
                        """
                        
                        # 創建完整的對話歷史
                        messages = [{"role": "system", "content": system_message}]
                        
                        # 添加之前的對話歷史（最多保留最近的5輪對話）
                        chat_history = st.session_state.chat_history[-10:]  # 限制歷史長度
                        ...
                        # 使用 OpenAI API 生成回答
                        client = openai.OpenAI(api_key="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")
                        ...
                        # 根據問題內容顯示相關圖表
                        if any(keyword in prompt.lower() for keyword in ['銷售', '營業額', '業績']):
                            with st.expander("📊 相關圖表", expanded=True):
                                # 繪製銷售趨勢圖
                                fig = px.box(df,
                                    x='Outlet_Identifier',
                                    y='Item_MRP',
                        ...
                        elif any(keyword in prompt.lower() for keyword in ['產品', '商品', '類別']):
                            with st.expander("📊 相關圖表", expanded=True):
                                # 繪製產品類別分布圖
                                fig = px.pie(df,
                                    values='Item_MRP',
                        ...
                        elif any(keyword in prompt.lower() for keyword in ['價格', '定價', 'mrp']):
                            with st.expander("📊 相關圖表", expanded=True):
                                # 繪製價格分布圖
                                fig = px.histogram(df,
                                    x='Item_MRP',
...
def get_chat_response(prompt, df):
    """使用 OpenAI API 生成回答"""
    try:
        # 準備系統消息
        system_message = """你是一個專業的數據分析助手，專門協助分析零售數據。
